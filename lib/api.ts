/** Delete a file by id from file-upload-service */
export const deleteLeadFile = async (fileId: string): Promise<void> => {
	if (!fileId) throw new Error("File id required for deletion");
	const response = await fetch(API_ENDPOINTS.FILE_DETAIL(fileId), { method: "DELETE", headers: apiHeaders() });
	if (!response.ok) {
...
export const API_ENDPOINTS = {
	...
	LEAD_PROPOSAL_ACCEPT: (id: string) => `${baseURL}/leads/${id}/proposal/accept`,
	LEAD_PROPOSAL_DECLINE: (id: string) => `${baseURL}/leads/${id}/proposal/decline`,
	LEAD_CONTRACT_SEND: (id: string) => `${baseURL}/leads/${id}/contract`,
	LEAD_CONTRACT_SIGNED: (id: string) => `${baseURL}/leads/${id}/contract/signed`,

	// Newsletter (owned by web-agency-backend-api)
	NEWSLETTER_SUBSCRIBE: `${baseURL}/newsletter/subscribe`,
	NEWSLETTER_CONFIRM: (token: string) => `${baseURL}/newsletter/confirm/${token}`,
...
export type UploadedLeadFile = { id: string; url: string; filename: string; mimeType?: string; size?: number };
...
export const submitContactForm = async (
	formData: any,
	options?: { attachments?: UploadedLeadFile[] },
	...
	const payload = {
		...
		customFields: {
			...
			...(options?.attachments?.length ?
				{ attachments: options.attachments, attachmentCount: options.attachments.length }
			:	{}),
		},
	};

	const result = await safePost<any>(API_ENDPOINTS.LEAD_SUBMIT, payload, { headers: apiHeaders(), timeout: 30000 });

	if (!result.success) throw new Error(result.error || "Failed to submit inquiry.");

	return {
		success: true,
		message:
			result.data?.message ?? "Thank you for your inquiry. We will review your project and get back to you soon.",
		id: result.data?.data?.id ?? result.data?.id,
	};
};

/**
 * Upload lead-related files via file-upload-service proxy endpoint.
 * Returns normalized file metadata that can be embedded into lead customFields.
 */
export const uploadLeadFiles = async (files: File[]): Promise<UploadedLeadFile[]> => {
	if (!files?.length) return [];

	const formData = new FormData();
	files.forEach((file) => formData.append("files", file));
	formData.append("category", "lead-attachment");

	const response = await fetch(API_ENDPOINTS.FILES_UPLOAD, { method: "POST", headers: apiHeaders(), body: formData });
	...
	if (!response.ok || !payload?.success) {
		const message = payload?.message || payload?.error?.message || "Failed to upload files.";
...
export const subscribeToNewsletter = async (email: string) => {
	if (!email || typeof email !== "string") throw new Error("Valid email address is required");
	...
	if (!result.success) throw new Error(result.error || "Failed to subscribe to newsletter.");
	return result.data;
};
...
export const confirmNewsletterSubscription = async (token: string) => {
	...
	if (!result.success) throw new Error(result.error || "Failed to confirm subscription.");
	return result.data;
};

/** Unsubscribe an email address from the newsletter. */
export const unsubscribeFromNewsletter = async (email: string, reason?: string, feedback?: string) => {
	if (!email) throw new Error("Email is required");
	const result = await safePost<any>(
		API_ENDPOINTS.NEWSLETTER_UNSUBSCRIBE,
		{ email: email.trim().toLowerCase(), ...(reason ? { reason } : {}), ...(feedback ? { feedback } : {}) },
...
export const uploadProposal = async (
	...
	const result = await safePost<any>(
		...
		{
			headers: apiHeaders(options?.accessToken ? { Authorization: `Bearer ${options.accessToken}` } : {}),
			timeout: 60000,
		},
	);
	if (!result.success) throw new Error(result.error || "Proposal upload failed.");
	return result.data;
};
...
export type AuthUser = { id: string; email: string; role: string; tenantId: string; sessionId?: string };

/** Login with email + password. Returns accessToken + refreshToken. */
export const loginUser = async (email: string, password: string): Promise<AuthTokens & { user?: AuthUser }> => {
	const result = await safePost<any>(
		API_ENDPOINTS.AUTH_LOGIN,
		{ email, password },
		{ headers: apiHeaders(), timeout: 15000 },
	);
	if (!result.success) throw new Error(result.error || "Login failed. Check your credentials.");
...
export const refreshAccessToken = async (token: string): Promise<string> => {
	...
	if (!result.success) throw new Error(result.error || "Session expired. Please log in again.");
	const data = result.data?.data ?? result.data;
...
export type LeadListParams = { page?: number; limit?: number; status?: string; search?: string };

/** Fetch paginated leads list. Requires a valid access token. */
export const getLeads = async (params: LeadListParams = {}, accessToken: string) => {
...
export const sendLeadProposal = async (
	...
	const result = await safePost<any>(API_ENDPOINTS.LEAD_PROPOSAL_SEND(leadId), data, {
		headers: { Authorization: `Bearer ${accessToken}`, ...apiHeaders() },
		timeout: 20000,
	});
	if (!result.success) throw new Error(result.error || "Failed to send proposal.");
	return result.data;
};
...
export const getProjectEstimate = async (params: CalculatorInput) => {
	...
	const result = await safePost<any>(API_ENDPOINTS.CALCULATOR_ESTIMATE, params, { headers: apiHeaders() });
	if (!result.success) throw new Error(result.error || "Estimate request failed.");
...
export const getCalculatorProfiles = async () => {
	const result = await safeGet<any>(API_ENDPOINTS.CALCULATOR_PROFILES, { headers: apiHeaders() });
	if (!result.success) throw new Error(result.error || "Failed to load profiles.");
	return result.data as {
		projectTypes: string[];
...
export const uploadFile = async (_file: File, _type: "resume" | "portfolio" | "image") => {
	throw new Error("Use uploadProposal() for proposals, or POST /api/files/upload for general file storage.");
};
...
export const sendEmail = async (_emailData: { to: string; subject: string; html: string; text?: string }) => {
	throw new Error("Email is dispatched server-side through the email-microservice.");
};
