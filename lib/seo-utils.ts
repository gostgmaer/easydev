...
export const generateSectionMeta = (section: string) => {
	...
	const sectionMeta = {
		...
		services: {
			...
			description: `Professional ${siteContent.services.list
				.map((s) => s.title)
				.slice(0, 3)
				.join(", ")} services by ${siteContent.personal.name}.`,
		},
		portfolio: {
			title: `Portfolio - ${siteContent.personal.name}`,
			description: `View ${siteContent.personal.name}'s latest projects and work in ${siteContent.portfolio.projects
				.map((p) => p.category)
				.slice(0, 3)
				.join(", ")}.`,
		},
...
export const generateProjectStructuredData = (project: any) => {
	return {
		...
		image: project.image,
		author: { "@type": "Person", name: siteContent.personal.name },
