...
export default function Footer() {
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEmail("");
      // Use the server message so users see "check your email" for double opt-in
      const successText =
        result?.message ||
        result?.data?.message ||
        "Please check your email to confirm your subscription.";
      setSubscriptionMessage({ type: "success", text: successText });
      ...
      try {
        ...
        console.warn(
          "⚠️ Failed to track subscription:",
            ...
            <div className="flex space-x-4">
              {siteContent.social.links.map((social) => {
                try {
                  ...
                  console.warn(
                    `⚠️ Error rendering social link for ${social.label}:`,
                    error,
                  );
                  return null;
                }
